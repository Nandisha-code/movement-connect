...
const Home = ({ organization }) => {
  ...
  const features = [
    {
      icon: BookOpen,
      title: organization.id === 'aisf' ? 'Education Rights' : 'Employment Rights',
      description: organization.id === 'aisf' 
    ...
    },
    {
      ...
      description: 'Bringing together young people from all backgrounds under the banner of progress and social justice.'
    },
    {
      icon: Megaphone,
      title: 'Grassroots Organizing',
      description: 'Building strong local networks to amplify the voice of the people at every level.'
    },
    {
      icon: Heart,
      title: 'Social Justice',
      description: 'Committed to creating an equitable society free from discrimination and exploitation.'
    },
  ];
    ...
    <main>
        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-12">
            {organization.campaigns.slice(0, 3).map((campaign, index) => (
              <article 
                key={campaign.id}
                className="bg-background/10 backdrop-blur-sm border border-background/20 rounded-xl p-6 hover:bg-background/20 transition-all animate-fade-up"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-background/60 text-sm">{campaign.year}</span>
                ...
                <h3 className="font-display text-xl text-background mb-3">{campaign.title}</h3>
                <p className="text-background/70 text-sm">{campaign.description}</p>
              </article>
            ))}
          </div>
          ...
          <div className="text-center">
            <Link
              ...
              className="inline-flex items-center gap-2 bg-background text-primary font-semibold px-8 py-4 rounded-lg hover:bg-background/90 transition-all group"
            >
              View All Campaigns
      ...
      <section className="section-padding bg-foreground">
        <div className="container-custom text-center">
          ...
          <p className="text-background/70 text-lg mb-8 max-w-2xl mx-auto animate-fade-up" style={{ animationDelay: '0.1s' }}>
            Join thousands of {organization.id === 'aisf' ? 'students' : 'youth'} across India who are working 
            together to build a more just and equitable society.
          </p>
          <div className="flex flex-wrap justify-center gap-4 animate-fade-up" style={{ animationDelay: '0.2s' }}>
            ...
            <Link
              to={`${basePath}/contact`}
              className="inline-flex items-center gap-2 bg-transparent text-background font-semibold px-8 py-4 rounded-lg border-2 border-background/30 hover:border-background hover:bg-background/10 transition-all"
            >
              Contact Us
            </Link>
          </div>
